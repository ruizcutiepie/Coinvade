...
import { tr, type LangCode } from './useLang';
...
export default function HeroKlinePanel({ lang }: { lang: LangCode }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!res.ok || data?.error) throw new Error(data?.message || data?.detail || 'Kline error');
        if (cancelled) return;
  ...
  const candleWidth =
    candles.length > 0 ? ((width - paddingX * 2) / candles.length) * 0.6 : 4;
  ...
  const labelHeroLastPrice = tHeroLastPrice === 'hero.lastPrice' ? 'Last Price' : tHeroLastPrice;
      ...
      <div className="rounded-3xl border border-cyan-500/25 bg-gradient-to-b from-slate-900/90 via-slate-950/95 to-black/95 p-5 shadow-[0_0_40px_rgba(0,255,255,0.18)]">
        <div className="mb-4 flex justify-between flex-wrap gap-4">
          ...
          <div className="flex flex-col items-end text-sm">
            ...
            >
              {lastPrice ? `${changePct >= 0 ? '+' : ''}${changePct.toFixed(2)}%` : '--'}
            </div>
          </div>
        </div>
        ...
        <div className="relative mt-2 rounded-2xl border border-white/5 bg-black/70 px-3 py-3">
          {hoverC && (
            <div className="absolute left-3 top-2 text-xs text-white/70">
              ...
              C:{' '}
              <span className={hoverC.close >= hoverC.open ? 'text-emerald-300' : 'text-rose-300'}>
                {fmt(hoverC.close)}
              </span>
            </div>
          )}
          ...
          >
            <defs>
              <pattern id="hero-grid" width="50" height="30" patternUnits="userSpaceOnUse">
                <path
                  d="M 50 0 L 0 0 0 30"
            ...
            {candles.map((c, idx) => {
              const x =
                paddingX +
                (idx / Math.max(1, candles.length - 1)) * (width - paddingX * 2);
            ...
            {candles.map((c, idx) => {
              ...
              const color =
                c.close >= c.open ? 'rgba(34,197,94,0.55)' : 'rgba(248,113,113,0.55)';
