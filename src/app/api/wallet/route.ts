...
// ✅ When Prisma expects Decimal in updates
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      // Ensure wallet exists for this coin
      ...
      const withdrawal = await tx.withdrawRequest.create({
        data: {
          ...
          note: note || undefined,
          status: 'PENDING' as any, // enum in schema, cast-safe
        } as any,
      });

      // Deduct from wallet now (reserve funds)
